...
- `src/app/auth/actions.ts` — `requestLoginCode` (signup + sign-in via `signInWithOtp`; before_user_created hook gates new MIT addresses), `verifyEmailOtp` (validates the 6-digit code via `verifyOtp({ type: 'email' })`, then redirects to `/`), and `signOut`. No password actions — passwordless by design.
...
8. **CRITICAL — make the "Magic Link" email template code-only.** Authentication → Email Templates → **Magic Link**. The body MUST render the 6-digit code `{{ .Token }}` and contain **NO** `{{ .ConfirmationURL }}` and no other clickable link. If a link is present, MIT Microsoft 365 Safe Links pre-fetches it and burns the shared OTP, so the code the user types *also* fails — the whole reason sign-in is code-based. (The separate **Change Email Address** template stays a link → `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=email_change&next=/`, used only by the alumni transition.) Verify by emailing yourself a code: the message must contain digits and no URL.
...
8. **Email templates (NOT optional — mirror Bootstrap step 8).** "Magic Link" template = **code only** (`{{ .Token }}`, no URL); a link there is silently consumed by MIT Safe Links and breaks every sign-in. "Change Email Address" template = link to `{{ .SiteURL }}/auth/confirm?token_hash={{ .TokenHash }}&type=email_change&next=/` (deliberate, rarely-used, to a usually-personal inbox; `/auth/confirm` handles it).
