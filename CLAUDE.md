...
- **Email domain gate has TWO layers now** (after a fail-open incident — see below):
  1. **`requestMagicLink` Server Action** rejects non-`@mit.edu`/`@alum.mit.edu` before calling Supabase. Fast UX + defense-in-depth. (Caveat: also blocks transitioned alumni from requesting a link; must become "allow known members" before the alumni-transition feature is actually used. Flagged in the code.)
  2. **`before_user_created` Auth Hook** (DB-side, the real boundary — also stops direct REST signups that skip the form).
...
4. **Enable the Auth Hook in the Dashboard**: Authentication → Hooks → "Before User Created" → Postgres function → schema `public`, function `before_user_created_check_mit_domain`. Save. *Backed up by the app-side check in `requestMagicLink`, but the hook is the only thing that stops direct REST signups. After enabling, run `supabase/health-check.sql` and confirm query 3 returns an `{"error":...}` object (NOT `{}` — `{}` means the gate is open).*
