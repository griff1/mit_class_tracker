...
- `src/app/page.tsx` — auth-aware home. Logged-out: marketing CTA. Logged-in: PageHeader + stats strip + **embedded class map** (was a separate `/map` route until we collapsed it) + nav tiles to Profile / Directory / Stats. Builds **two** geo aggregations from one query — `cities` (Lives here) and `visiting_cities` (Frequently visits) — and passes both to `<ClassMap livesHere visits>`; an unmapped-city note is rendered per layer. `/map` redirects to `/` via `next.config.ts`.
